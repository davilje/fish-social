...
- 默认窗口尺寸为 `960×560`（09D；07B 起曾为 `960×480`）。
...
- 渲染 Unity 推送的鱼塘场景、钓位、自己的猫和（07C）同塘玩家；猫咪 **显示 `64×64`**（源图 `256×256`）。悬停时长浮窗热区同为猫身 64×64。
...
- [x] 启动后窗口为 `960×560`、无边框、透明、置顶（09D 正式尺寸；07G 验收时曾为 `960×480`）。
...
- [x] 状态序列号可防止旧状态覆盖新状态。
- [x] Windows Development Build 通过主窗口、鱼塘、Overlay 启停和退出测试。

## 7. 变更记录

| 日期 | 作者 | 变更 |
|------|------|------|
| 2026-08-29 | 策划 | 口径对齐 09D/ART-03：窗口 `960×560`；猫显示 64×64、源图 256×256；悬停热区为猫身 |
