...
- [ ] Unity 可查看好友列表和在线状态。
- [ ] 可创建 Lobby 并写入 `pondId`。
- [ ] 好友可通过邀请加入 Lobby。
- [ ] 加入 Lobby 后能读取正确的 `pondId`。
- [ ] Node 能校验 Lobby、玩家和鱼塘权限。
- [ ] 双方可进入同一个 Node 鱼塘 Socket 会话。
- [ ] 关闭 Lobby、鱼塘不存在、版本不兼容和无权限时有明确提示。
- [ ] 简化版 UI 可替换，不影响 Controller、Adapter 和服务端协议。
- [ ] 不使用 Steam Networking/Relay。
- [ ] 不把权威玩家数据写入 Steam Lobby。
- [ ] 真实 Steam 测试与本地模拟测试分开。
- [ ] Lobby 关闭不删除鱼塘。
- [ ] 房主离开不关闭鱼塘。
- [ ] Lobby 失效后，已有鱼塘仍可从地图/好友入口进入。
- [ ] 最后一个玩家离开后，鱼塘进入休眠并停止高频 Tick。
- [ ] 重新进入同一 `pondId` 前，按 `last_simulated_at` 完成幂等补算。
- [ ] 不创建每鱼塘常驻服务器进程。
