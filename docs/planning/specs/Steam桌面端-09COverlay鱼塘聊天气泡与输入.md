...
- [x] Overlay 可见最近鱼塘公屏消息（至少含昵称+文本）
- [x] 输入框可发送 ≤200 字；空消息不可发
- [x] 发送成功后自己与他人 Overlay 均能看到新气泡
- [x] 断线/服务端拒绝有 `errorMessage` 或 toast
- [x] 不 `leave_pond`；钓鱼操作栏仍可正常点击
- [x] 主窗口 PanelSocial 聊天仍完整可用，与 Overlay 消息一致
- [x] 私聊不在 Overlay 出现
...
| 日期 | 说明 |
|------|------|
| 2026-08-20 | 用户验收通过 → **已实现**；Overlay 公屏气泡 + 紧凑输入 + IPC `recentChats` / `send_pond_chat` |
| 2026-08-20 | 开发交付：Overlay 左下公屏气泡流 + 底部紧凑输入；完整聊天仍主窗口 |
