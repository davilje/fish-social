...
请在 `fish-social-unity/` 内实现 Fish Social 产品区域的右键菜单，并接入原生 Overlay 的右键菜单命令。
...
- `960×480` Overlay 右键必须能打开菜单。
- 菜单中的“打开主窗口”必须显示 `1280×720` 主窗口并恢复焦点，可隐藏 Overlay，但不得离开鱼塘。
...
- 菜单操作不会触发 `leave_pond`。
- 菜单关闭和主窗口恢复不会创建重复 Overlay 或重复鱼塘会话。
