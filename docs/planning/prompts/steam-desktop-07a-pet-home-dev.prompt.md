...
已验收的原生 `FishSocialOverlay.exe` 由 07G 提供，本任务负责按正式窗口流程调用它。
...
- 启动后显示自己的 2D 猫咪宠物。
- 应用启动时先显示 `480×320` 登录窗口，只有一个“Steam 登录”按钮。
- Steam 登录成功后显示默认 `1280×720` 主窗口。
- 主窗口显示小猫状态栏，侧边栏改为底部横向按钮行。
...
- 提供进入/恢复当前鱼塘入口。
- 提供返回桌面宠物主视图入口。
- 点击进入/恢复鱼塘时调用原生 Overlay 并隐藏主窗口。
- 主窗口与 Overlay 使用同一份自己的猫状态模型。
...
- 不在本需求中重写 07G 的原生窗口和 IPC；只调用其稳定接口并推送自己的猫状态。
...
- 可进入鱼塘并返回桌面宠物视图。
- 状态切换不会创建重复会话。
- 登录窗口尺寸为 `480×320`，登录成功后主窗口尺寸为 `1280×720`。
- 主窗口底部显示横向导航按钮行。
- 进入鱼塘后主窗口隐藏，Overlay 显示且自己的猫状态一致。
