...
EVENTS = [
    ...
    ("D成长进度", "fish_returned_to_pond", "metrics", "回鱼入塘增重并发奖", "playerId\npondId", '{"speciesId":"crucian","sizeM":0.2,"gold":56,"sizeGainM":0.03,"auto":false}', "P0", "已实现", "returnFish.ts"),
    ("D成长进度", "return_fee_mode_selected", "metrics", "进塘双价模式选择", "playerId\npondId", '{"mode":"auto_return","feePer2h":350}', "P1", "已实现", "socketPondHandlers.ts"),
    ("D成长进度", "fish_auto_returned", "metrics", "达标鱼自动回塘", "playerId\npondId", '{"speciesId":"crucian","sizeM":0.28,"gold":120,"auto":true}', "P1", "已实现", "returnFish.ts"),
    ("D装备", "groundbait_cast_started", "metrics", "开始打窝扣金", "playerId\ngroundbaitId", '{"costGold":30,"stackBefore":0}', "P1", "已实现", "groundbait.ts"),
    ("D装备", "groundbait_applied", "metrics", "打窝完成叠层", "playerId\npondId", '{"stackAfter":1,"biteBonus":0.006}', "P1", "已实现", "groundbait.ts"),
    ("D装备", "groundbait_rejected", "metrics", "打窝拒绝", "playerId\nreason", '{"reason":"gold","code":"INSUFFICIENT_GOLD"}', "P1", "已实现", "groundbait.ts"),
    ("D成长进度", "achievement_unlocked", "metrics", "成就解锁", "playerId\nachievementId", '{"achievementId":"ach-first-catch"}', "P1", "已实现", "achievements.ts"),
    ("D成长进度", "album_pin_changed", "metrics", "相册钉选变更", "playerId\npinCount\naction", '{"pinCount":3,"action":"pin"}', "P1", "已实现", "album.ts"),
    ("D成长进度", "profile_hub_opened", "metrics", "打开个人中心", "playerId\ntab", '{"tab":"album"}', "P1", "已实现", "profileHub.ts"),
